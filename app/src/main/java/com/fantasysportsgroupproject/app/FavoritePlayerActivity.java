package com.fantasysportsgroupproject.app;

import android.app.Activity;
import android.os.Bundle;
import android.view.Menu;
import android.view.MenuItem;

import com.fantasysportsgroupproject.app.Helpers.PlayerSearchListArrayAdapter;
// This screen will be called from Anna's screen when you
// click favorites button
// create favorites table in Parse so that the favorites can be stored


// Check the difference between activity and List activity
public class FavoritePlayerActivity extends Activity {

    PlayerSearchListArrayAdapter adapter;

    @Override
    public void onStart() {
        super.onStart();

        // Update mAdapter with favorites
        adapter.clear();
        // Load favorites from parse
       // adapter.addAll(FavoritesUtil.readFavorites(this));
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {

        super.onCreate(savedInstanceState);
        //setContentView(R.layout.activity_favorite_player);
        setContentView(R.layout.player_list);

        /* uncomment later
        adapter = new PlayerSearchListArrayAdapter(this,new ArrayList<PlayerList>());
        setListAdapter(adapter);
        */
    }




    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        
        // Inflate the menu; this adds items to the action bar if it is present.
        getMenuInflater().inflate(R.menu.favorite_player, menu);
        return true;
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        // Handle action bar item clicks here. The action bar will
        // automatically handle clicks on the Home/Up button, so long
        // as you specify a parent activity in AndroidManifest.xml.
        int id = item.getItemId();
        if (id == R.id.action_settings) {
            return true;
        }
        return super.onOptionsItemSelected(item);
    }

}
